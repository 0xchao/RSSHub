...
module.exports = async (ctx) => {
    ...
    const out = await Promise.all(
        list.map(async (item, i) => {
            ...
            let title = $('h4.title a').text();
            const itemUrl = 'https://abduzeedo.com' + $('h4.title a').attr('href');
    ...
    for (let i = 0; i < responses.length; i++) {
        ...
        let time = $('.node__submitted p:nth-child(2)').text();
        ...
        let author = $('.written-by article h5 a').text();
