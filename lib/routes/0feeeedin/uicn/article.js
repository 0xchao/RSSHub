...
module.exports = async (ctx) => {
    ...
    const result = await Promise.all(
        postList.map(async (item) => {
            ...
            const guid = link;
            const imageUrlAll = $(item).find('.cover img').attr('src');
            const imageUrl = `<img src="http:` + imageUrlAll + `">`;
            const single = {
                title: title,
                link: link,
                guid: guid,
            ...
            if (description_value && pubDate_value) {
                ...
                single.description = single.imageUrl + $(temp.data).find('.works-cont').html();
