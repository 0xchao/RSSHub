...
module.exports = async (ctx) => {
    ...
    const out = await Promise.all(
        list.map(async (item, i) => {
            ...
            let title = $('.box-info .content .row h3 a').text();
    ...
    for (let i = 0; i < responses.length; i++) {
        ...
        let time = $('span[itemprop="datePublished"]').attr('content');
        out[indexList[i]].pubDate = new Date(time).toUTCString();
        let title = $('h1[itemprop="headline name"]').text();
        ...
        let category = $('span[itemprop="about"]').text();
        out[indexList[i]].category = category;
        let author = $('span[itemprop="name author"]').text();
