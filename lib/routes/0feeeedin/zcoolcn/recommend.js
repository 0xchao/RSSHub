...
module.exports = async (ctx) => {
    ...
    const list = $('.work-list-box > .card-box')
        .map((i, e) => {
            ...
            const imageUrlAll = element.find('.card-img-hover').find('img').attr('src').replace('@260w_195h_1c_1e_1o_100sh.jpg', '');
            const imageUrl = `<img src="` + imageUrlAll + `">`;
    ...
    const result = await Promise.all(
        list.map(async (item) => {
            ...
            item.description = item.imageUrl + itemElement('.work-content-wrap').html();
